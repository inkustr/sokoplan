...
def plot_01_box_violin(common: List[str], gnn: Dict[str, Row], hun: Dict[str, Row], out: Path) -> None:
    ...
    axes[0].set_title("Community packs: nodes distribution")
    ...
    axes[1].set_title("Community packs: runtime distribution")
...
def plot_02_scatter(common: List[str], gnn: Dict[str, Row], hun: Dict[str, Row], out: Path) -> None:
    ...
    ax.set_title("Community packs: nodes scatter (log-log)")
    ax.grid(alpha=0.25)
    ...
    ax.set_title("Community packs: runtime scatter (log-log)")
    ax.grid(alpha=0.25)
    ax.legend(frameon=True, fontsize=8, loc="upper left")
...
def plot_03_ecdf_nodes(common: List[str], gnn: Dict[str, Row], hun: Dict[str, Row], out: Path) -> None:
    ...
    ax.set_ylabel("ECDF")
    ax.set_title("Community packs: ECDF of expanded nodes")
    ax.grid(alpha=0.25)
...
def plot_04_solution_delta(common: List[str], gnn: Dict[str, Row], hun: Dict[str, Row], out: Path) -> None:
    ...
    ax.set_xlabel("Delta solution length (GNN pushes - Hungarian pushes)")
    ax.set_ylabel("levels (%)")
    ax.set_title("Community packs: solution-length delta (percentage of levels)")
...
def plot_05_sorted_runtime(common: List[str], gnn: Dict[str, Row], hun: Dict[str, Row], out: Path) -> None:
    ...
    ax.set_title("Community packs: sorted runtime curves")
    ax.grid(alpha=0.25)
    ax.legend(frameon=True)
    _save(fig, out)
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Generate 5 evaluation plots for Community packs: GNN vs Hungarian."
    )
    p.add_argument("--gnn_csv", default="results/evaluate/letslogic/final/gnn.csv")
    p.add_argument("--hun_csv", default="results/evaluate/letslogic/final/hungarian.csv")
