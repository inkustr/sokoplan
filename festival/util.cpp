...
void exit_with_error(const char *message)
{
	...
	// Check if we're in non-interactive mode (set by Python wrapper)
	char *disable_interactive = getenv("FESTIVAL_DISABLE_INTERACTIVE");
	if (disable_interactive == NULL || strcmp(disable_interactive, "1") != 0) {
		my_getch();
	}
	
	exit(1);  // Exit with error code 1 instead of 0
}
...
int get_number_of_cores()
{
	...
	memory = (unsigned long long)pages * page_size / 1073741824;
	printf("detected %lld GB memory\n", memory);
#endif
